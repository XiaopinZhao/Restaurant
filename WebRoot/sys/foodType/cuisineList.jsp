				...
				<c:if test="${ not empty requestScope.FoodTypeList }">
					<c:forEach var="item" items="${requestScope.FoodTypeList }">
						<tr>
							...
							<td>${item.typeName }</td>
							<td>
								<a href="${pageContext.request.contextPath }/FoodTypeServlet?method=updataFoodType&id=${item.id}" class="FunctionButton">更新</a> 
								<a href="${pageContext.request.contextPath }/FoodTypeServlet?method=deleteFoodType&id=${item.id}" class="FunctionButton">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:if>
